...
export default publicProcedure
  ...
  .mutation(async ({ ctx, input }) => {
    ...
    if (!event?.event_id || event.approval_status !== "approved") {
      throw new Error("Choose an approved event before listing a car.");
    }
    if (isRideShareEventExpired(event)) {
      throw new Error("This run has already ended, so ride-share cars can no longer be listed.");
    }
    ...
    const { data, error } = await ctx.supabase
      ...
      .insert({
        ...
        status: input.vehicleType === "passenger_car_light" ? "active" : "pending_approval",
      })
      .select("ride_offer_id")
      .single();

    if (error) throw new Error(error.message || "Could not list your car.");

    return { success: true, offerId: data.ride_offer_id };
  });
